...
it.layer(NodeServices.layer)("skillCatalogFingerprint", (it) => {
  ...
  it.effect("changes when SKILL.md contents are rewritten", () =>
    Effect.gen(function* () {
      const fileSystem = yield* FileSystem.FileSystem;
      const path = yield* Path.Path;
      const tempDir = yield* fileSystem.makeTempDirectoryScoped({
        prefix: "t3-skill-fingerprint-edit-",
      });
      const skillsDir = path.join(tempDir, "skills");
      yield* writeSkill(skillsDir, "review", "---\ndescription: one\n---\n");
      const before = yield* fingerprintSkillCatalogRoots([skillsDir]);
      yield* fileSystem.writeFileString(
        path.join(skillsDir, "review", "SKILL.md"),
        "---\ndescription: updated\n---\n",
      );
      const after = yield* fingerprintSkillCatalogRoots([skillsDir]);
