...
import { ChatAttachment, OrchestrationQuickAction } from "@t3tools/contracts";
...
const ProjectionThreadMessageDbRowSchema = ProjectionThreadMessage.mapFields(
  Struct.assign({
    ...
    quickActions: Schema.NullOr(Schema.fromJsonString(Schema.Array(OrchestrationQuickAction))),
  }),
);
...
function toProjectionThreadMessage(
  ...
  return {
    ...
    ...(row.quickActions !== null ? { quickActions: row.quickActions } : {}),
  };
}
...
const makeProjectionThreadMessageRepository = Effect.gen(function* () {
  ...
  const upsertProjectionThreadMessageRow = SqlSchema.void({
    ...
    execute: (row) => {
      ...
      const nextQuickActionsJson =
        row.quickActions !== undefined ? JSON.stringify(row.quickActions) : null;
      return sql`
        ...
        VALUES (
          ...
          COALESCE(
            ${nextQuickActionsJson},
            (
              SELECT quick_actions_json
              FROM projection_thread_messages
              WHERE message_id = ${row.messageId}
            )
          ),
        ...
        DO UPDATE SET
          ...
          quick_actions_json = COALESCE(
            excluded.quick_actions_json,
            projection_thread_messages.quick_actions_json
          ),
    ...
    execute: ({ messageId }) =>
      sql`
        SELECT
          ...
          quick_actions_json AS "quickActions",
